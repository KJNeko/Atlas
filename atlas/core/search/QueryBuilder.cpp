			...
			switch ( type )
			{
				...
				default:
					throw std::runtime_error(
						std::format( "{}: Failed to process \"{}\" in switch statement", __func__, str ) );
			}
		}
	}

	throw std::runtime_error( std::format( "{}: Failed to process \"{}\"", __func__, str ) );
}
	...
	for ( const auto& [ text, type ] : namespaces )
	{
		if ( str.starts_with( text ) )
		{
			...
			else
			{
				...
				{
					case CREATOR:
						return std::format( " creator LIKE \'{}\'", escape( sub ) );
					case ENGINE:
						return std::format( " engine LIKE \'{}\'", escape( sub ) );
					case TITLE:
						return std::format( " title LIKE \'{}\'", escape( sub ) );
					case TAG:
						return std::format(
...
std::string parse( const std::string_view str )
{
	...
	if ( isNamespace( str ) )
		...
		throw std::runtime_error( std::format( "Failed to parse! {}", str ) );
}
