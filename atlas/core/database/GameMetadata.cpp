...
struct VersionKey
{
	...
	bool operator==( const VersionKey& other ) const { return id == other.id && version_name == other.version_name; }
};

template <>
struct std::hash< VersionKey >
{
	std::size_t operator()( const VersionKey& data ) const noexcept
	{
...
VersionData::VersionData( const RecordID record_id, const QString& name ) : m_parent_id( record_id ), m_version( name )
{
	RapidTransaction()
