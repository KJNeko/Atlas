...
void Game::reorderPreviews( std::vector< std::filesystem::path > paths )
{
	...
	{
		RapidTransaction() << "INSERT INTO previews (record_id, path, position) VALUES (?,?,?)" << m_id << paths[ i ]
						   << i;
	}
...
void Game::removeUserTag( QString str )
{
	...
	if ( const auto itter = std::find( tags.begin(), tags.end(), str ); itter != tags.end() )
	{
		RapidTransaction() << "DELETE FROM tag_mappings NATURAL JOIN WHERE tag = ? AND record_id = ?" << str << m_id;
		tags.erase( itter );
	}
}
