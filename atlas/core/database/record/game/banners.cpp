...
namespace atlas::records
{
	...
	{
		if ( future.isFinished() || future.isCanceled() ) return future;
		...
		return future
		    .onFailed(
				[ info = std::move( closure ) ]( const std::exception& e ) -> QPixmap
				{
					atlas::logging::error(
		...
		const bool use_thumbnail )
	{
		...
		if ( use_thumbnail )
		{
			...
			return createFailureHandler( future, id, banner_path, type );
		}
		else
		{
			QFuture< QPixmap > future {
