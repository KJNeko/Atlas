...
void loadImage( QPromise< QPixmap >& promise, const std::filesystem::path path )
{
	...
	if ( promise.isCanceled() ) return;
	if ( pixmap.size() == QSize( 0, 0 ) || pixmap.isNull() ) spdlog::warn( "Suspected failed to load banner {}", path );
	promise.addResult( pixmap );
}
	...
	const std::filesystem::path path )
{
	...
	else
	{
		...
		{
			atlas::logging::
				userwarn( fmt::format( "Qt failed to load image {} Pixmap was null after attempted loading. ", path ) );
			promise.addResult( pixmap );
		}
